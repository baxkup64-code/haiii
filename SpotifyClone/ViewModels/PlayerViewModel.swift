...
final class PlayerViewModel: ObservableObject {
    ...
    func togglePlayPause() {
        ...
        if track.source == .spotify {
            SpotifyPlaybackService.shared.authorizeAndPlay(trackID: track.id)
            return
        }
    ...
    private func loadCurrent() {
        ...
        case .soundcloud:
            if let streamURL = track.streamURL {
                let resolved = Track(id: track.id, title: track.title, artistName: track.artistName, albumName: track.albumName, artworkURL: track.artworkURL, streamURL: streamURL, sourceURL: track.sourceURL, duration: track.duration, source: track.source)
                queue[currentIndex] = resolved
                audio.load(track: resolved)
            } else {
                Task {
                    guard let streamURL = await MusicAPIService.shared.resolveSoundCloudStream(trackID: track.id) else {
                        await MainActor.run { self.isFullPlayerPresented = true }
                        return
                    }
                    guard self.currentIndex < self.queue.count, self.queue[self.currentIndex].id == track.id else { return }
                    await MainActor.run {
                        let updated = Track(id: track.id, title: track.title, artistName: track.artistName, albumName: track.albumName, artworkURL: track.artworkURL, streamURL: streamURL, sourceURL: track.sourceURL, duration: track.duration, source: track.source)
                        self.queue[self.currentIndex] = updated
                        self.audio.load(track: updated)
                    }
                }
            }
        case .spotify:
